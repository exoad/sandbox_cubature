...
typedef int (*evalError_func)(struct rule_s *r, unsigned fdim, integrand_v f,
...
static int eval_regions(unsigned nR, region *R, integrand_v f, void *fdata,
                        rule *r) {
  unsigned iR;
 ...
 */
static void evalR_Rfs(double *pts, unsigned dim, double *p, const double *c,
                      const double *r) {
  unsigned i;
...
static void evalRR0_0fs(double *pts, unsigned dim, double *p, const double *c,
                        const double *r) {
  unsigned i, j;
...
#undef VALS
  ...
  for (iR = 0; iR < nR; ++iR) {
    ...
    for (j = 0; j < fdim; ++j) df += R[iR].ee[j].err;
    ...
    for (i = 0; i < dim; ++i) {
      ...
      if (delta > df) {
        ...
        dimDiffMax = i;
      } else if (fabs(delta) <= df &&
                 R[iR].h.data[dim + i] > R[iR].h.data[dim + dimDiffMax])
        dimDiffMax = i;
    }
...
static int rule15gauss_evalError(rule *r, unsigned fdim, integrand_v f,
                                 void *fdata, unsigned nR, region *R) {
  ...
  for (k = 0; k < fdim; ++k) {
    ...
    for (iR = 0; iR < nR; ++iR) {
      ...
      for (j = 0; j < (n - 1) / 2; ++j) {
        ...
        result_abs +=
            wgk[j2] * (fabs(vk[fdim * npts]) + fabs(vk[fdim * npts + fdim]));
        npts += 2;
      }
      for (j = 0; j < n / 2; ++j) {
        int j2 = 2 * j;
...
static int converged(unsigned fdim, const esterr *ee, double reqAbsError,
                     double reqRelError, error_norm norm)
...
#include "converged.h"
    ...
    static int rulecubature(rule *r, unsigned fdim, integrand_v f, void *fdata,
  ...
  while (numEval < maxEval || !maxEval) {
    if (converged(fdim, regions.ee, reqAbsError, reqRelError, norm)) break;
...
static int cubature(unsigned fdim, integrand_v f, void *fdata, unsigned dim,
  ...
  status = !h.data ? FAILURE
                   : rulecubature(r, fdim, f, fdata, &h, maxEval, reqAbsError,
...
int hcubature_v(unsigned fdim, integrand_v f, void *fdata, unsigned dim,
                const double *xmin, const double *xmax, size_t maxEval,
                double reqAbsError, double reqRelError, error_norm norm,
                double *val, double *err) {
  return cubature(fdim, f, fdata, dim, xmin, xmax, maxEval, reqAbsError,
                  reqRelError, norm, val, err, 1);
}

#include "vwrapper.h"

int hcubature(unsigned fdim, integrand f, void *fdata, unsigned dim,
              const double *xmin, const double *xmax, size_t maxEval,
              double reqAbsError, double reqRelError, error_norm norm,
              double *val, double *err) {
  int ret;
  ...
  d.f = f;
  d.fdata = fdata;
  ret = cubature(fdim, fv, &d, dim, xmin, xmax, maxEval, reqAbsError,
