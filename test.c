...
double fconst(double x[], size_t dim, void *params) { return 1; }
...
static double morokoff(unsigned dim, const double *x, void *params) {
  ...
  unsigned int i;
  for (i = 0; i < dim; i++) prod *= pow(x[i], p);
  return prod;
}

/*** end of GSL test functions ***/

int f_test(unsigned dim, const double *x, void *data_, unsigned fdim,
           double *retval) {
  double val;
  unsigned i, j;
  ++count;
...
static double exact_integral(int which, unsigned dim, const double *xmax) {
  ...
  switch (which) {
    case 0:
      val = 1;
      for (i = 0; i < dim; ++i) val *= sin(xmax[i]);
      break;
    case 2:
...
int main(int __, char **____) {
  ...
  if (argc <= 3) {
    ...
    for (i = 0; argv[3][i]; ++i) {
      if (argv[3][i] == '/')
        ...
        which_integrand[j] = which_integrand[j] * 10 + argv[3][i] - '0';
      else {
        fprintf(stderr, "invalid which_integrand \"%s\"", argv[3]);
        return EXIT_FAILURE;
      }
  ...
  cubature(integrand_fdim, f_test, NULL, dim, xmin, xmax, maxEval, 0, tol,
